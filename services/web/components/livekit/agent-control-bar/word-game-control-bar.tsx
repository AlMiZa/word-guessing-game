...
// Total words in the game (20 fallback words per language)
const TOTAL_WORDS_IN_GAME = 20;

/**
 * Control bar for the word guessing game.
 * Features language selection, start/stop game buttons, and score display.
 */
export function WordGameControlBar({
  className,
  ...props
  ...
  const [score, setScore] = useState({
    correct: 0,
    total: TOTAL_WORDS_IN_GAME,
  });

  // Listen for score updates from agent via data channel
  useEffect(() => {
  ...
  useEffect(() => {
    if (room.state === 'disconnected') {
      ...
      setScore({ correct: 0, total: TOTAL_WORDS_IN_GAME });
    }
  }, [room.state]);
  ...
  const handleSkipQuestion = useCallback(async () => {
    ...
    try {
      ...
      // Call the skip_question RPC method on the agent
      await room.localParticipant.performRpc({
        destinationIdentity: agent.identity,
        method: 'skip_question',
        payload: '',
      });

      // Don't auto-increment score - let the agent's response update it
    } catch (error) {
      ...
      {!gameStarted && (
        <div className="flex flex-col gap-2">
          <label
            ...
            className="text-fg1 flex items-center gap-2 text-sm font-medium"
          >
      ...
      {gameStarted && (
        <div className="flex items-center justify-between rounded-md border border-bg2 bg-bg1 px-4 py-3">
          ...
          <div className="text-fg1 text-xl font-bold">
            ...
            <span className="text-fg2 ml-2 text-sm font-normal">
              ({accuracy}%)
            </span>
          </div>
        </div>
        ...
        {!gameStarted ? (
          ...
          <>
            <div className="text-fg2 text-sm text-center">
              Playing in{' '}
              <span className="font-semibold text-fg1">{selectedLanguage}</span>
            </div>
            <div className="flex gap-2">
              <Button
                ...
                disabled={isLoading}
                className="font-mono md:h-10"
              >
                <SkipForwardIcon weight="bold" className="mr-2" />
                NEXT
              </Button>
              <Button
                variant="destructive"
